<?php
include("db.php"); 
include("tags_to_set.php");

// TODO : add a GUI for this to be usable easily.
...
if (count($tags_1) != count($tags_2)){
    exit('<span style="padding-left:40px;color:red;">ERROR : different number of tags. Aborting.</span></br>');
}
if (count($tags_1) == 0 || count($tags_2) == 0){
    exit('<span style="padding-left:40px;color:red;">ERROR : one of the tag lists is empty. Aborting.</span></br>');
}
...
for ($i=0; $i<$tag_count; $i++){
    ...
    mysql_query($q_str);
    echo '<span style="padding-left:40px;color:green;">UPDATED</span></br>';
